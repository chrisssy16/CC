...
<body>
  ...
  <header>
    ...
    <div class="container">
        <h1 class="my-4">Your Jobs</h1>
        <div class="card">
    <div class="list-group">
        @forelse($jobs ?? [] as $job)
            <div class="list-group-item">
                <h2 class="mb-1"><strong>{{ $job->job_title }}</strong> </h2>
                <p class="mb-1">{{ $job->job_description }}</p>
                <p class="mb-1"><strong>Category:</strong> {{ $job->category }}</p>
                <p class="mb-1"><strong>Budget Range:</strong> {{ $job->budget_range }}</p>
                <p class="mb-1"><strong>Deadline:</strong> {{ $job->deadline }}</p>
                <div class="d-flex justify-content-between align-items-center">
                    <a href="{{ route('job.edit', $job) }}" class="btn btn-primary btn-sm mr-2">Edit</a>
                    <form action="{{ route('job.delete', $job) }}" method="post" class="d-inline">
                        @csrf
                        @method('delete')
